...
def _request(base, token, path, method="GET", body=None, auth=True):
    url = f"{base}/jupyter_sidekick/{path}"
    headers = {}
    if auth:
...
async def test_stream_websocket_rejects_unauthenticated(server):
    ...
    base, _token = server
    ws_url = base.replace("http://", "ws://") + "/jupyter_sidekick/chats/x/stream"
...
from jupyter_sidekick.handlers import resolve_cwd  # noqa: E402
